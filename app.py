...
# Manejo seguro para similitud de nombres
try:
...
# ==========================================
# LISTA DE API KEYS (TU NUEVA CLAVE ESTÁ DE PRIMERA Y ACTIVA)
# ==========================================
API_KEYS = [
    "14db0e108529faed46c94b3163188df5",  # Clave Nueva Principal
    "3e69e51ac95c094a672f790edac978b0"   # Clave Anterior de Respaldo
]
...
class FootballAPI:
    def __init__(self):
        self.keys = [k.strip() for k in API_KEYS if k and "PEGA_AQUI" not in k and len(k) > 10]
        self.key_errors = {}
        self.ultimo_error = ""

    def consultar(self, endpoint: str, params: dict) -> list:
        if not self.keys:
            self.ultimo_error = "🛑 No hay API Keys válidas configuradas."
            return []
        ...
        self.ultimo_error = f"🛑 Error de API: {detalles if detalles else 'Revisa la activación de tu clave en api-football.com'}"
        return []
    ...
    def buscar_partido(self, local: str, visitante: str, fecha: str):
        ...
        eq_l = self.buscar_equipo(local)
        eq_v = self.buscar_equipo(visitante)
        return {
...
class SALMEngine:
    ...
    def ejecutar_analisis(self, local: str, visitante: str, fecha: str, liga: str) -> Match:
        ...
        if pr["o15"] >= 68.0: cands.append({"m": "Más de 1.5 Goles Totales", "p": pr["o15"], "r": "Bajo", "razon": f"Expectativa Poisson de {pr['exp']:.2f} goles."})
        ...
        arg = (
            ...
            f"**2. Proyección Poisson:** Gol Local: {pr['lh']:.2f} | Gol Visita: {pr['lv']:.2f} (Total: {pr['exp']:.2f} goles).\n\n"
            f"**3. Historial H2H Real:** Promedio H2H de {an['prom_h2h']:.1f} goles.\n\n"
            f"**4. Dictamen Exclusivo:** {p_top['razon']}"
        )

        return Match(local=loc_n, visitante=vis_n, market_ranking=cands, main_prediction=p_top["m"], alternative_prediction=s_top["m"], explanation=arg)
