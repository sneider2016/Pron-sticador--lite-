...
def limpiar_nombre_busqueda(t: str) -> str:
    """
    Extrae el nombre principal del club cortando sufijos 'de [Ciudad/País]'.
    Ejemplos:
    - 'Nacional de Montevideo' -> 'Nacional'
    - 'Tigre de Argentina' -> 'Tigre'
    ...
    # Cortar por palabras clave de separación ' de ' o ' del '
        ...
        if not analisis_raw.get("datos_reales_ok"):
            return Match(
                ...
                explanation=f"**Atención:** No fue posible obtener el historial de partidos reales para **{local}** o **{visitante}** desde API-Football.\n\n"
                            f"💡 **Sugerencia para buscar de nuevo:** Escribe solo el nombre principal del equipo (ejemplo: 'Nacional' en lugar de 'Nacional de Montevideo', o 'Tigre' en lugar de 'Tigre de Argentina').",
...
if st.session_state.get("analizado", False):
    ...
    # BOTÓN DESTACADO PARA VOLVER A BUSCAR FÁCILMENTE
    if st.button("🔄 Realizar Nueva Búsqueda", key="btn_top_reset"):
        st.session_state.clear()
        st.rerun()
    ...
    if p_top["p"] > 0:
        ...
        c_betplay = st.number_input(f"Cuota actual en Betplay para '{target_market['m']}':", min_value=1.01, max_value=20.0, value
