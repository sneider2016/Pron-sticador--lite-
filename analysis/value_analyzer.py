class ValueAnalyzer:
    ...
    def decidir(self, ev: float, ratio_discrepancia: float) -> str:
        # VÁLVULA DE SEGURIDAD ANTI-TRAMPA (Anti-PSV)
        if ratio_discrepancia > 1.35:
            return "🛑 NO APOSTAR (Discrepancia Sospechosa con la Casa)"
        if ev >= 0.05:
    ...
    def calcular_kelly(self, probabilidad: float, cuota: float, fraccion: float = 0.20) -> dict:
        """
        Criterio de Kelly con Techo Duro (Flat Cap) para blindar el bankroll.
        """
        ...
        # CAP MÁXIMO DE STAKE: Máximo $15.000 COP por apuesta para evitar que una pérdida borre 5 victorias
